...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remixBlueStack-d172
  fly apps create remixBlueStack-d172-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remixBlueStack-d172
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remixBlueStack-d172-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remixBlueStack-d172-staging' does not match app name in config file 'remixBlueStack-d172'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remixBlueStack-d172-db
  fly postgres attach --app remixBlueStack-d172 remixBlueStack-d172-db

  fly postgres create --name remixBlueStack-d172-staging-db
  fly postgres attach --app remixBlueStack-d172-staging remixBlueStack-d172-staging-db
  ```
